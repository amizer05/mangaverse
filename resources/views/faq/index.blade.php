...
<div class="bg-gradient-to-b from-slate-900 to-slate-950 py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Search Box -->
        <div class="mb-12">
            <form method="GET" action="{{ route('faq.index') }}" class="relative max-w-2xl mx-auto" x-data="{ focused: false }">
                ...
                <input type="text" 
                       ...
                       autocomplete="off">
                @if($query ?? null)
                <a href="{{ route('faq.index') }}" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-white transition-colors" title="Clear search">
                ...
                <div x-show="focused" 
                     ...
                     style="display: none;">
                    <kbd class="px-2 py-1 bg-slate-700 rounded">Enter</kbd> to search
                </div>
            </form>
            @if($query ?? null)
        ...
        @if(!($query ?? null))
        <div class="mb-8 flex flex-wrap gap-3 justify-center" x-data="{ activeCategory: null }">
            <button @click="activeCategory = null; document.querySelectorAll('.faq-category').forEach(el => el.style.display = 'block')"
            ...
            <button @click="activeCategory = {{ $catIndex }}; document.querySelectorAll('.faq-category').forEach((el, i) => el.style.display = i === {{ $catIndex }} ? 'block' : 'none')"
        ...
        <div class="space-y-8">
            ...
            <div class="faq-category bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden hover:border-indigo-500/50 transition-all duration-300">
                ...
                <div class="divide-y divide-slate-700" x-data="{ openItem: null }">
                    @foreach($category->faqItems as $index => $item)
                    <div class="faq-item hover:bg-slate-800/30 transition-colors duration-200">
                        <!-- Question -->
                        <button @click="openItem = openItem === {{ $catIndex }}_{{ $index }} ? null : {{ $catIndex }}_{{ $index }}"
                            ...
                            <svg class="w-6 h-6 text-slate-400 group-hover:text-indigo-400 flex-shrink-0 transition-all duration-300 ml-4"
                                 :class="openItem === {{ $catIndex }}_{{ $index }} ? 'rotate-180 text-indigo-400' : ''"
                        ...
                        <!-- Answer -->
                        <div x-show="openItem === {{ $catIndex }}_{{ $index }}"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="question" rows="3" required
                                  ...
                                  placeholder="What would you like to know?">{{ old('question') }}</textarea>
                        @error('question')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <button type="submit" class="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-indigo-600 to-rose-600 hover:from-indigo-500 hover:to-rose-500 rounded-xl text-white font-semibold transition-all transform hover:scale-105 shadow-lg flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-12 bg-gradient-to-r from-indigo-600 via-purple-600 to-rose-600 rounded-2xl p-8 md:p-12 text-center relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <a href="{{ route('contact.create') }}" class="inline-flex items-center px-8 py-4 bg-white hover:bg-slate-100 rounded-xl text-indigo-600 font-semibold transition-all duration-200 transform hover:scale-105 shadow-2xl">
